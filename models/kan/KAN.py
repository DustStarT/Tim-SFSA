...
class KAN(nn.Module):
    ...
    def __init__(self, width=None, grid=3, k=3, noise_scale=0.1, noise_scale_base=0.1, base_fun=torch.nn.SiLU(),
        ...
        for l in range(self.depth):
            # splines
            scale_base = 1 / np.sqrt(width[l]) + (torch.randn(width[l] * width[l + 1], device=device) * 2 - 1) * noise_scale_base
            sp_batch = KANLayer(
    ...
    def forward(self, x):
        ...
        for l in range(self.depth):
            ...
            if self.symbolic_enabled:
                ...
                x_symbolic = torch.tensor(0., device=self.device)
                postacts_symbolic = torch.tensor(0., device=self.device)

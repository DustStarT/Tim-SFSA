...
class KANLayer(nn.Module):
    ...
    def __init__(self, in_dim=3, out_dim=2, num=5, k=3, noise_scale=0.1, scale_base=1.0, scale_sp=1.0,
        ...
        if isinstance(scale_base, float):
            ...
            self.scale_base = torch.nn.Parameter(torch.tensor(scale_base, dtype=torch.float32, device=device).clone().detach()).requires_grad_(sb_trainable)
    ...
    def forward(self, x):
        ...
        # 核心修复：确保x的维度与grid的维度完全匹配。
        # grid的维度是 (size, grid_points)，其中 size = out_dim * in_dim
        # 我们需要将x从 (batch, in_dim) 转换为 (size, batch)
        
        # 1. 将输入 x 扩展以匹配输出维度
        # (batch, in_dim) -> (batch, out_dim, in_dim)
        x = x.unsqueeze(1).expand(batch, self.out_dim, self.in_dim)
        
        # 2. 将扩展后的张量转置并重塑，使其第一个维度是样条/特征组合
        # (batch, out_dim, in_dim) -> (out_dim, in_dim, batch)
        x = x.permute(1, 2, 0)
        
        # 3. 最终扁平化为 (out_dim * in_dim, batch)，即 (size, batch)
        x = x.reshape(self.out_dim * self.in_dim, batch)

        preacts = x.reshape(self.out_dim, self.in_dim, batch).permute(2,0,1)

        # 现在 x_eval 的形状是 (size, batch)，与 grid (size, grid_points) 匹配
        y = coef2curve(x_eval=x, grid=self.grid[self.weight_sharing],
                       coef=self.coef[self.weight_sharing], k=self.k, device=self.device)

        # 检查NaN值
        if torch.isnan(y).any():
            logging.warning('[KANLayer] y contains NaN!')

        base = self.base_fun(x)

        # y 和 base 的形状都是 (size, batch)
        # 缩放样条和基函数
        y = self.scale_sp.unsqueeze(1) * y + self.scale_base.unsqueeze(1) * base

        # 应用掩码
        y = self.mask.unsqueeze(1) * y

        # 存储激活值
        postacts = y.reshape(self.out_dim, self.in_dim, batch).permute(2,0,1)
        postspline = (self.scale_sp.unsqueeze(1) * y).reshape(self.out_dim, self.in_dim, batch).permute(2,0,1)
        
        # 最后，将结果重塑为 (batch, out_dim)
        y = y.reshape(self.out_dim, self.in_dim, batch).sum(dim=1)
        y = y.permute(1,0)

        # 检查最终输出的NaN值
        if torch.isnan(y).any():
            logging.warning('[KANLayer] final y contains NaN!')
    ...
    def get_subset(self, in_id, out_id):
        ...
        spb.coef.data = self.coef.reshape(
            self.out_dim, self.in_dim, spb.coef.shape[1])[out_id][:, in_id].reshape(-1, spb.coef.shape[1])

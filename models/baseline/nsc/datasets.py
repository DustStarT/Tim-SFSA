...
def load_dataset(dataset='SUPPORT', **kwargs):
    ...
    # canonicalize durations to hours for downstream consumers
    try:
        durations_hours = _ensure_durations_in_hours(np.asarray(df['duration'].values.astype(float)), cfg=None, name='models_baseline_nsc_load_dataset')
    except Exception:
        durations_hours = np.asarray(df['duration'].values.astype(float))
